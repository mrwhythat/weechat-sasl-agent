...
def nist256p_challenge(key, challenge):
    """
    Compute NIST256P signature over the given challenge data

    Assume 'challenge' data is base64-encoded string and 'key'
    is a base64-encoded private key to be used for signing.
    """
    # TODO: add if challenge == '+' logic (see weechat's NIST256P impl.)
    backend = default_backend()
    data = base64.b64decode(challenge)
    private_key = load_pem_private_key(key.encode('ascii'), None, backend)
...
def main(args):
    ...
    SASL DELEGATED protocol logic:
        ...
        - if called with two arguments, assume it's path to the key and
    ...
    if len(args) == 1:
        ...
        keyfile, challenge = args[1], args[2]
        with open(keyfile, 'r') as f:
            print(nist256p_challenge(f.read(), challenge))
    else:
        print('')
